...
(function () {
  ...
  Disputes: {
    ...
    open: async function(againstId, sessionId, orderId, reason, evidenceUrls) {
      var res = await fetch(FUNCTIONS_URL + '/manage-disputes?action=open', {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({
  ...
  Students: {
    search: async function(skills, minRating, limit) {
      ...
      var res = await fetch(FUNCTIONS_URL + '/platform-jobs?' + params.toString(), {
        headers: authHeaders()
      });
      return await res.json();
    }
